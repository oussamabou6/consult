...
// Build the query to get withdrawal requests
$sql_count = "SELECT COUNT(*) as total FROM withdrawal_requests w 
              JOIN users u ON w.user_id = u.id";

$sql = "SELECT w.*, u.full_name, u.email, u.balance 
...
if (!empty($date_from)) {
    $where_clauses[] = "DATE(w.created_at) >= ?";
    $params[] = $date_from;
    $types .= "s";
}
...
// Add a new query to get total experts balance after the existing stats query
$total_experts_balance_sql = "SELECT SUM(balance) as total_balance FROM users WHERE role = 'expert'";
$total_experts_balance_result = $conn->query($total_experts_balance_sql);
$total_experts_balance = ($total_experts_balance_result && $total_experts_balance_result->num_rows > 0) ? $total_experts_balance_result->fetch_assoc()['total_balance'] : 0;
...
<head>
    ...
    <title>Withdrawal Requests - <?php echo htmlspecialchars($site_name); ?></title>
    ...
    <style>
        ...
        * {
        ...
        .stat-card.approved::before, .stat-card.completed::before {
        ...
        .stat-card.approved .stat-card-title i, .stat-card.completed .stat-card-title i {
            color: var(--success-color);
        }
        ...
        .stat-card.approved .stat-card-value, .stat-card.completed .stat-card-value {
            color: var(--success-color);
        }
...
<body>
 ...
 <a href="notifications.php" class="menu-item">
        ...
        <div class="main-content">
            ...
            <!-- Statistics -->
            <!-- Replace the existing stat cards HTML with this -->
<div class="stats-container">
    <div class="stat-card total" title="Click to view all requests">
        <div class="stat-card-title">
            <i class="fas fa-money-bill-wave"></i> Total Requests
        </div>
        <div class="stat-card-value"><?php echo $stats['total_requests'] ?? 0; ?></div>
        <div class="stat-card-desc">All withdrawal requests</div>
    </div>
    ...
    <div class="stat-card completed" title="Click to filter by completed status">
        <div class="stat-card-title">
            <i class="fas fa-check-circle"></i> Completed
        </div>
        <div class="stat-card-value"><?php echo $stats['completed_requests'] ?? 0; ?></div>
        <div class="stat-card-desc">Approved withdrawals</div>
    </div>
    <div class="stat-card rejected" title="Click to filter by rejected status">
        ...
        <div class="stat-card-desc">Declined withdrawals</div>
    </div>
    ...
    <div class="stat-card total">
        <div class="stat-card-title">
            <i class="fas fa-users-cog"></i> Experts Balance
        </div>
        <div class="stat-card-value"><?php echo number_format($total_experts_balance ?? 0, 2); ?></div>
        <div class="stat-card-desc"><?php echo $currency; ?> available</div>
    </div>
    <div class="stat-card completed">
        <div class="stat-card-title">
            <i class="fas fa-check-double"></i> Completed Amount
        </div>
        <div class="stat-card-value"><?php echo number_format($stats['completed_amount'] ?? 0, 2); ?></div>
        <div class="stat-card-desc"><?php echo $currency; ?> withdrawn</div>
    </div>
</div>
            ...
            <div class="filters">
                <form method="GET" class="search-form">
                    ...
                    <input type="text" name="search" class="search-input" placeholder="Search by expert name or email" value="<?php echo htmlspecialchars($search); ?>">
                    ...
                    <div class="filter-buttons">
                        ...
                        <?php if (!empty($search) || $status_filter != "all" || !empty($date_from) || !empty($date_to)): ?>
                            <a href="withdrawal-requests.php" class="btn btn-warning">
                    ...
                    <table class="withdrawals-table">
                        ...
                        <tbody>
                            <?php while ($row = $result->fetch_assoc()): ?>
                                <?php 
                                    $commission_amount = $row['amount'] * ($commission_rate / 100);
                                    $final_amount = $row['amount'] - $commission_amount;
                                ?>
                                <tr>
                                    ...
                                    <td><?php echo number_format($commission_amount, 2); ?> <?php echo $currency; ?> (<?php echo $commission_rate; ?>%)</td>
                                    <td><strong><?php echo number_format($final_amount, 2); ?> <?php echo $currency; ?></strong></td>
                                    ...
                                    <td class="action-buttons">
                                        ...
                                        <?php if ($row['status'] == 'pending'): ?>
                                            <a href="view-withdrawal.php?id=<?php echo $row['id']; ?>&action=approve" class="btn btn-success btn-sm">
                                                <i class="fas fa-check"></i> <span>Approve</span>
                                            </a>
                                            <a href="view-withdrawal.php?id=<?php echo $row['id']; ?>&action=reject" class="btn btn-danger btn-sm">
...
document.querySelector('.stat-card.total').addEventListener('click', function() {
    window.location.href = 'withdrawal-requests.php';
});
document.querySelector('.stat-card.pending').addEventListener('click', function() {
    window.location.href = 'withdrawal-requests.php?status=pending';
});
document.querySelector('.stat-card.completed').addEventListener('click', function() {
    window.location.href = 'withdrawal-requests.php?status=completed';
});
document.querySelector('.stat-card.rejected').addEventListener('click', function() {
    window.location.href = 'withdrawal-requests.php?status=rejected';
});
        });
    </script>
</body>
